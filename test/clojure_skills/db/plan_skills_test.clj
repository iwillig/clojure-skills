...
(deftest associate-skill-with-plan-test
  (testing "associate-skill-with-plan with valid data"
    ...
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Test Plan for Skills"})
          plan-id (:implementation_plans/id test-plan)]

      ;; Create test skill
      (jdbc/execute! tu/*test-db*
      ...
      (let [skill-id (-> (jdbc/execute-one! tu/*test-db* ["SELECT id FROM skills ORDER BY id LIMIT 1"])
                         :skills/id)
            result (plan-skills/associate-skill-with-plan tu/*test-db*
                                                          {:plan-id plan-id
                                                           :skill-id skill-id
                                                           :position 1})]
  ...
  (testing "associate-skill-with-plan with default position"
    ;; Create test plan and skills first
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Test Plan 2"})
          plan-id (:implementation_plans/id test-plan)]
      ...
      (let [skill-id (-> (jdbc/execute-one! tu/*test-db* ["SELECT id FROM skills ORDER BY id DESC LIMIT 1"])
                         :skills/id)
            result (plan-skills/associate-skill-with-plan tu/*test-db*
                                                          {:plan-id plan-id
                                                           :skill-id skill-id})]
  ...
  (testing "associate-skill-with-plan with duplicate throws"
    ;; Create test plan and skills first
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Test Plan 3"})
          plan-id (:implementation_plans/id test-plan)]
...
(deftest dissociate-skill-from-plan-test
  (testing "dissociate-skill-from-plan removes association"
    ;; Create test plan and skills first
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Test Plan 4"})
          plan-id (:implementation_plans/id test-plan)]
      ...
      (let [skill-id (-> (jdbc/execute-one! tu/*test-db* ["SELECT id FROM skills ORDER BY id DESC LIMIT 1"])
        ...
        ;; Dissociate
        (let [result (plan-skills/dissociate-skill-from-plan tu/*test-db*
                                                             {:plan-id plan-id
                                                              :skill-id skill-id})]
          ...
          ;; Verify removed
          (let [skills (plan-skills/list-plan-skills tu/*test-db* plan-id)]
            (is (empty? skills)))))))

  (testing "dissociate-skill-from-plan with non-existent association returns 0"
    ;; Create test plan and skills first
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Test Plan 5"})
          plan-id (:implementation_plans/id test-plan)

          ;; Create test skill
          _ (jdbc/execute! tu/*test-db*
                           ["INSERT INTO skills (path, category, name, content, file_hash, size_bytes)
                             VALUES (?, ?, ?, ?, ?, ?)"
                            "/test/skill5.md" "test" "test-skill-5" "Test content 5" "hash5" 500])
          skill-id (-> (jdbc/execute-one! tu/*test-db* ["SELECT id FROM skills ORDER BY id DESC LIMIT 1"])
                       :skills/id)
          result (plan-skills/dissociate-skill-from-plan tu/*test-db*
                                                         {:plan-id plan-id
                                                          :skill-id skill-id})]
...
(deftest list-plan-skills-test
  (testing "list-plan-skills returns empty for plan with no skills"
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Empty Plan"})
          plan-id (:implementation_plans/id test-plan)
          result (plan-skills/list-plan-skills tu/*test-db* plan-id)]
      (is (empty? result))))

  (testing "list-plan-skills returns associated skills"
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Skills Plan"})
          plan-id (:implementation_plans/id test-plan)]
      ...
      (let [skills (jdbc/execute! tu/*test-db* ["SELECT id FROM skills ORDER BY id DESC LIMIT 3"])
        ...
        (let [result (plan-skills/list-plan-skills tu/*test-db* plan-id)]
          (is (= 3 (count result)))
  ...
  (testing "list-plan-skills with invalid plan-id throws"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          ...
                          (plan-skills/list-plan-skills tu/*test-db* "not-an-int")))

    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          #"plan-id must be >= 1"
                          (plan-skills/list-plan-skills tu/*test-db* 0)))))
...
(deftest get-skill-by-name-test
  (testing "get-skill-by-name returns existing skill"
    ...
    (let [result (plan-skills/get-skill-by-name tu/*test-db* "get-by-name-skill")]
      (is (some? result))
  ...
  (testing "get-skill-by-name with non-existent name returns nil"
    (is (nil? (plan-skills/get-skill-by-name tu/*test-db* "does-not-exist"))))

  (testing "get-skill-by-name with empty string throws"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          #"Validation failed"
                          (plan-skills/get-skill-by-name tu/*test-db* "")))))
...
(deftest get-skill-by-path-test
  (testing "get-skill-by-path returns existing skill"
    ...
    (let [result (plan-skills/get-skill-by-path tu/*test-db* "/test/get-by-path.md")]
      (is (some? result))
  ...
  (testing "get-skill-by-path with non-existent path returns nil"
    (is (nil? (plan-skills/get-skill-by-path tu/*test-db* "/does/not/exist.md"))))

  (testing "get-skill-by-path with empty string throws"
    (is (thrown-with-msg? clojure.lang.ExceptionInfo
                          #"Validation failed"
                          (plan-skills/get-skill-by-path tu/*test-db* "")))))
...
(deftest full-lifecycle-test
  (testing "complete plan-skill association lifecycle"
    (let [test-plan (plans/create-plan tu/*test-db* {:name "Lifecycle Test Plan"})
          plan-id (:implementation_plans/id test-plan)]
